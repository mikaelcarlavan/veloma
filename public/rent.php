...
$startDate = GETPOST('start-date', 'alpha');
$endDate = GETPOST('end-date', 'alpha');
...
if ($mode == 'book') {
    ...
    if (!empty($startDate)) {
        if (is_numeric($startDate)) {
            ...
            $start = dol_stringtotime($startDate, 0);
        }
    }
    ...
    if (!empty($endDate)) {
        if (is_numeric($endDate)) {
            ...
            $end = dol_stringtotime($endDate, 0);
        }
    }
}
    ...
    foreach ($bikes as $bike) {
        if ($bike->fk_stand > 0) {
            if (!isset($bookingsByBike[$bike->id])) {
                $bike->photo = dol_buildpath('/veloma/public/photo.php?modulepart=bike&id='.$bike->id, 1);
        ...
        <div class="mt-10 sm:flex sm:justify-center lg:justify-center">
            <form class="w-3/5" id="confirm" name="confirm" action="<?php echo dol_buildpath('/veloma/public/rent.php', 1); ?>" method="post">
                ...
                <div class="mt-2 py-3 sm:flex sm:flex-row-reverse">
                    ...
                    <a href="<?php echo dol_buildpath('/veloma/public/rent.php?mode='.$mode, 1); ?>" class="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"><?php echo $langs->trans('VelomaCancel'); ?></a>
                </div>
            </form>
        </div>
    <?php else: ?>
